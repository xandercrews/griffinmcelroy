...
class WebHDFS(object):
    ...
    def __init__(self, namenode_host, namenode_port, datanode_host, datanode_port, hdfs_username):
    ...
    def put(self, source_data, target_path, replication=1):
        ...
        fileUploadClient = httplib.HTTPConnection(self.datanode_host,
                                                  self.datanode_port, timeout=600)
    ...
    def get(self, source_path):
        ...
        if response.length!=None:
            ...
            logger.debug("Send redirect to: host: %s, port: %s, path: %s "%(redirect_host, redirect_port, redirect_path))
            fileDownloadClient = httplib.HTTPConnection(self.datanode_host,
                                                        self.datanode_port, timeout=600)
